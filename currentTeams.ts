...
export async function getCurrentTeams(): Promise<{
    ...
    const safelyGetTeams = async (getter: () => Promise<TeamInfo>, errorMessage: string) => {
        try {
    ...
    const dataForNonSKAHLSite = [
        safelyGetTeams(
            ...
            "SKAHL Pond Data Not Currently Available"
        ),
        safelyGetTeams(
            () => getCurrentKHLTeams(),
            "KHL Data Not Currently Available"
        ),
    ];
    ...
    const dataForSKAHLSite = allCurrentSKAHLSeasons.map((season) =>
        safelyGetTeams(
            ...
            `${season.name} data not currently available.`
        )
    );
