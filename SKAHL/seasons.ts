...
type ServerTeamInfo = {name: string, divisionName: string, teamId: string, seasonId: string};
export class SKAHLSeason {
    ...
    private async getTeamsFromServer(): Promise<ServerTeamInfo[]> {
        return await get(`http://snokinghockeyleague.com/api/team/list/${this.id}/0?v=1021270`) as Promise<ServerTeamInfo[]>;
    }
...
export async function getCurrentSeasons() {
    ...
    return allSeasons.filter(s => s.name.indexOf(currentYear) >= 0);
}